module VariableMultiRecorder #(BITWIDTH = 8, type WriteType) (
        input [3:0][BITWIDTH-1:0] inputVals, input [3:0] valids, 
        input clk, rst, enb, 
        output WriteType ramOut
    );

    logic [$clog2(BITWIDTH):0] numOnes;
    ...
    always_comb begin
        ...
        ramOut.enb = enb && (&valids[3:2]); // disable if no write occurs to save some power (i think it'll save, at least)
    ...
    always_ff @(posedge clk)begin
        if(rst) baseAddress <= '0;
            ...
            baseAddress <= baseAddress + numOnes; // increase by however many valid vals we just wrote
        end
    end
